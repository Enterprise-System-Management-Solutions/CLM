--
-- SP_LD_CLV_LD_MANUAL  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_CAMPAIGN_USER01.SP_LD_CLV_LD_MANUAL (P_DATE VARCHAR2) IS
MIN_VREVENUE NUMBER;
MAX_VREVENUE NUMBER;
MIN_DREVENUE NUMBER;
MAX_DREVENUE NUMBER;
...
FROM
(
SELECT MSISDN,NVL(VOICE_REVENUE,0)+NVL(DATA_REVENUE,0) AS TOTAL_VREVENUE
FROM
( SELECT MSISDN,SUM(NVL(VOICE_REVENUE,0))VOICE_REVENUE,SUM(NVL(DATA_REVENUE,0))DATA_REVENUE
FROM
(
(SELECT MSISDN,SUM(VOICE_REVENUE) AS VOICE_REVENUE, null as DATA_REVENUE 
FROM CLM_VOICE_REV@CAMPAING_TO_DWH05
WHERE DATE_KEY=VDATE_KEY
group by MSISDN)
UNION ALL
(SELECT MSISDN, NULL AS VOICE_REVENUE ,SUM(DATA_REVENUE)AS DATA_REVENUE
FROM CLM_DATA_REV@CAMPAING_TO_DWH05
WHERE DATE_KEY=VDATE_KEY
group by MSISDN)
)
